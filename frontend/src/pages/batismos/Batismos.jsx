...
function ModalBatizar({ onFechar }) {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end lg:items-center justify-center p-0 lg:p-4">
      <div className="bg-white w-full lg:max-w-md lg:rounded-2xl max-h-[95vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-3">
          <div>
            ...
            <select value={membroId} onChange={e => setMembroId(e.target.value)}
              ...
              {pendentes.map(m => <option key={m.id} value={m.id}>{m.nome}</option>)}
            </select>
          </div>
          <div>
        ...
        <div className="flex gap-2 p-4 border-t sticky bottom-0 bg-white">
          ...
          <button onClick={() => batizar.mutate()} disabled={batizar.isPending || !membroId || !data}
            className="flex-1 bg-blue-600 text-white rounded-lg py-2 text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
            {batizar.isPending ? 'Salvando...' : 'Registrar'}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">{data.length} batismos registrados</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-gray-400"><Droplets size={48} className="mx-auto mb-2 opacity-30" /><p>Nenhum batismo registrado</p></div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-600 hidden lg:table-cell">Local</th>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {b.foto_url
                        ...
                        : <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-xs font-bold text-blue-700">{b.membro_nome?.[0]}</div>
                      }
                      <span className="font-medium">{b.membro_nome}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-gray-500">{formatarData(b.data)}</td>
                  <td className="px-4 py-3 text-gray-500 hidden lg:table-cell">{b.local || '—'}</td>
                  <td className="px-4 py-3 text-gray-500 hidden lg:table-cell">{b.pastor_nome || '—'}</td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => { if (confirm('Remover batismo?')) deletar.mutate(b.id); }} className="p-1.5 text-gray-400 hover:text-red-600 rounded"><Trash2 size={15} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {modal && <ModalBatizar onFechar={() => setModal(false)} />}
    </div>
  );
}
